...
class InterestUseCase @Inject constructor(private val profileRepository: ProfileRepository) {
    suspend fun execute(interest: String) = update(interest)
    suspend operator fun invoke(interest: String) = update(interest)
    private suspend fun update(interest: String) = profileRepository.updateInterest(interest)
}